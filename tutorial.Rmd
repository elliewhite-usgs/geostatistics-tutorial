...
setwd("/Users/EllieWhite/Box Sync/HYD273-Geostatistics_Fogg_Puente/Project")
```
...
Use block kriging to smoothe out the surface, and have a less computationally expensive model.  
...
# 9.0 Comparing Models  
To recap, the models developed are: Average Model(am), Inverse Distance Weighted (idw), Simple Kriging (sk), Ordinary Kriging (ok), Unversal Kriging with a Linear Trend (ukl), Unversal Kriging with a Quadratic Trend (ukq), Indicator Kriging (ik), Co-Kriging (ck), Block Kriging (bk). 
...
# the kriging algorithm will approximate the conditional probabilities by considering the twelve nearest neighbours for all points in the simulation grid.
